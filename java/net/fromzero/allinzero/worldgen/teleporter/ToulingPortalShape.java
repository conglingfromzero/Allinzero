package net.fromzero.allinzero.worldgen.teleporter;
...
public class ToulingPortalShape {
    ...
    public ToulingPortalShape(Direction.Axis p_77697_, int p_374222_, Direction p_374407_, BlockPos p_77696_, int p_374218_, int p_374477_) {
        this.axis = p_77697_;
    ...
    public static Optional<ToulingPortalShape> findEmptyPortalShape(LevelAccessor p_77709_, BlockPos p_77710_, Direction.Axis p_77711_) {
        return findPortalShape(p_77709_, p_77710_, (p_77727_) -> p_77727_.isValid() && p_77727_.numPortalBlocks == 0, p_77711_);
    }

    public static Optional<ToulingPortalShape> findPortalShape(LevelAccessor p_77713_, BlockPos p_77714_, Predicate<ToulingPortalShape> p_77715_, Direction.Axis p_77716_) {
        Optional<ToulingPortalShape> optional = Optional.of(findAnyShape(p_77713_, p_77714_, p_77716_)).filter(p_77715_);
    ...
    public static ToulingPortalShape findAnyShape(BlockGetter p_374054_, BlockPos p_374346_, Direction.Axis p_374516_) {
        Direction direction = p_374516_ == Axis.X ? Direction.WEST : Direction.SOUTH;
        BlockPos blockpos = calculateBottomLeft(p_374054_, direction, p_374346_);
        if (blockpos == null) {
            return new ToulingPortalShape(p_374516_, 0, direction, p_374346_, 0, 0);
        } else {
            int i = calculateWidth(p_374054_, blockpos, direction);
            if (i == 0) {
                return new ToulingPortalShape(p_374516_, 0, direction, blockpos, 0, 0);
            } else {
                MutableInt mutableint = new MutableInt();
                int j = calculateHeight(p_374054_, blockpos, direction, i, mutableint);
                return new ToulingPortalShape(p_374516_, mutableint.getValue(), direction, blockpos, i, j);
            }
        }
    }
    ...
    private static int getDistanceUntilTop(BlockGetter p_374443_, BlockPos p_374231_, Direction p_374062_, BlockPos.MutableBlockPos p_77729_, int p_374313_, MutableInt p_374330_) {
        for(int i = 0; i < 21; ++i) {
            ...
            for(int j = 0; j < p_374313_; ++j) {
                ...
                if (blockstate.getBlock() == AllinzeroBlocks.TOULING_PORTAL.get()) {
                    p_374330_.increment();
                }
            }
        }
    ...
    private static boolean isEmpty(BlockState p_77718_) {
        return p_77718_.isAir() || p_77718_.getBlock() == AllinzeroBlocks.TOULING_PORTAL.get();
    }
    ...
    public void createPortalBlocks(LevelAccessor p_374419_) {
        BlockState blockstate = (BlockState)((Block)AllinzeroBlocks.TOULING_PORTAL.get()).defaultBlockState().setValue(NetherPortalBlock.AXIS, this.axis);
    ...
    public static Vec3 getRelativePosition(BlockUtil.FoundRectangle p_77739_, Direction.Axis p_77740_, Vec3 p_77741_, EntityDimensions p_77742_) {
        ...
        double d2;
        if (d0 > (double)0.0F) {
            double d3 = (double)blockpos.get(p_77740_) + (double)p_77742_.width() / (double)2.0F;
            d2 = Mth.clamp(Mth.inverseLerp(p_77741_.get(p_77740_) - d3, (double)0.0F, d0), (double)0.0F, (double)1.0F);
        } else {
            d2 = (double)0.5F;
        }
        ...
        double d5;
        if (d1 > (double)0.0F) {
            Direction.Axis direction$axis = Axis.Y;
            d5 = Mth.clamp(Mth.inverseLerp(p_77741_.get(direction$axis) - (double)blockpos.get(direction$axis), (double)0.0F, d1), (double)0.0F, (double)1.0F);
        } else {
            d5 = (double)0.0F;
        }

        Direction.Axis direction$axis1 = p_77740_ == Axis.X ? Axis.Z : Axis.X;
        double d4 = p_77741_.get(direction$axis1) - ((double)blockpos.get(direction$axis1) + (double)0.5F);
    ...
    public static Vec3 findCollisionFreePosition(Vec3 p_260315_, ServerLevel p_259704_, Entity p_259626_, EntityDimensions p_259816_) {
        if (!(p_259816_.width() > 4.0F) && !(p_259816_.height() > 4.0F)) {
            double d0 = (double)p_259816_.height() / (double)2.0F;
            Vec3 vec3 = p_260315_.add((double)0.0F, d0, (double)0.0F);
            VoxelShape voxelshape = Shapes.create(AABB.ofSize(vec3, (double)p_259816_.width(), (double)0.0F, (double)p_259816_.width()).expandTowards((double)0.0F, (double)1.0F, (double)0.0F).inflate(1.0E-6));
